 ...
 * @return html
 */
if (!function_exists('count_indicator_widget')) {

    function count_indicator_widget($user_id = 0) {
        $ci = new \App\Models\Indicators\Rating();
        $options = array(
            "user_id" => $user_id
        );
        $info = $ci->getRatingSum($user_id);
        $view_data["count_ball"] = round($info["rating_sum"], 1) ?? 0;
        $template = new Template();
        return $template->view("indicators/widgets/count_indicator_widget", $view_data);
    }
...
/**
 * activity logs widget for reports
 * @param array $params
 * @return html
 */
if (!function_exists('activity_logs_widget_reports')) {

    function activity_logs_widget_reports($params = array()) {
        $ci = new Security_Controller(false);
        $re = new \App\Controllers\Reports();

        $data = $re->listNotification();

        $template = new Template();
        return $template->view("activity_logs/activity_logs_widget_reports", $data);
    }
