...
class Security_Controller extends App_Controller {
    ...
    protected function can_indicator_list() {
        if (!$this->login_user->is_admin && get_array_value($this->login_user->permissions, "can_indicator_list") != "1") {
            app_redirect("forbidden");
        }
    }

    protected function can_indicator_add_edit_delete() {
        if (!$this->login_user->is_admin && get_array_value($this->login_user->permissions, "can_indicator_add_edit_delete") != "1") {
            app_redirect("forbidden");
        }
    }

    protected function can_send_reports_view() {
        if (!$this->login_user->is_admin && get_array_value($this->login_user->permissions, "can_send_reports_view") != "1") {
            app_redirect("forbidden");
        }
    }

    protected function can_send_reports_approved(): bool {
        if (!$this->login_user->is_admin && get_array_value($this->login_user->permissions, "can_send_reports_approved") != "1") {
            return false;
        }
        return true;
    }
    ...
    protected function get_team_members_only_admins_dropdown(int $is_admin = 0, $is_filter = false) {
        $team_members = $this->Users_model->get_all_where(array("user_type" => "staff", "is_admin" => $is_admin, "deleted" => 0, "status" => "active"))->getResult();
